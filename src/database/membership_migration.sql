...
ALTER TABLE users ADD COLUMN IF NOT EXISTS membership_type TEXT DEFAULT 'FREE' CHECK (membership_type IN ('FREE', 'MEMBER'));
...
-- Function to check if user is MEMBER member
CREATE OR REPLACE FUNCTION is_user_member(user_uuid UUID)
RETURNS BOOLEAN AS $$
BEGIN
  RETURN EXISTS (
    SELECT 1 
