...
-- Create user logs table to track app events
CREATE TABLE IF NOT EXISTS user_logs (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  timestamp TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  category TEXT NOT NULL CHECK (category IN ('AUTH', 'DATA', 'TASK', 'FRIEND', 'SYSTEM', 'ERROR')),
...
CREATE TABLE IF NOT EXISTS tasks (
  ...
  status TEXT NOT NULL DEFAULT 'PENDING' CHECK (status IN ('PENDING', 'IN_PROGRESS', 'COMPLETED', 'OVERDUE', 'GRADED')),
...
-- RLS policies for user logs
CREATE POLICY "Users can view their own logs"
  ON user_logs
  FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "System can insert logs for any user"
  ON user_logs
  FOR INSERT
  WITH CHECK (true);
